...
def setup_telemetry() -> str | None:
    ...
    # LOGS_BUCKET_NAME is the documented on-switch: setting it enables upload in
    # NO_CONTENT (metadata-only) mode. Capture mode defaults to NO_CONTENT but
    # stays overridable by the explicit env if an operator widened it — the old
    # gate required that env to be pre-set, so the obvious knob did nothing.
    if bucket:
