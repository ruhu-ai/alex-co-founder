...
| Founder Drive connector | OAuth `drive.readonly` + `drive.file`; the founder selects specific readable files (no blanket indexing), and `drive.file` can create only app-produced exports. Refresh tokens minted in-app persist to Secret Manager on Cloud Run / to `.env` locally, never to state or logs. |
...
| Alex's role account (adr/001) | Separate Workspace account (`alex@ruhu.ai`) and connector-specific refresh-token slots. Mail uses `gmail.modify` for ordinary read/send/label/archive operations but excludes settings, delegation and immediate permanent deletion. Alex Drive uses `drive` for full management of files owned by the dedicated role account. Alex Calendar uses `calendar.readonly` + `calendar.events` for calendar reads and event management, excluding calendar ACL/sharing/deletion. Durable workflow authority, recipient/thread ownership, idempotency, audit, uncertainty and kill switches remain the authorization boundary; OAuth scope alone never authorizes an effect. Any scope-contract change requires fresh consent, and older narrower grants fail closed. Inbound content is extraction-only and never followed as instructions. |
...
| Connector registry | Connectors are data (`services/connectors.py`), not UI code; the panel renders `GET /api/v1/connectors`; adding one never bypasses the per-connector scope checks |
