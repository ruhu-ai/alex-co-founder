...
restore, or founder request. It is a projection for the exact owned conversation
being viewed, not a second global inbox. It is not generated by a model and does
not poll.
...
4. **Safe inbox:** current actor-authorized ambiguity, failure, connection, and
   follow-up items only when their durable origin or resolved delivery session
   exactly matches the conversation being viewed. Unlinked workspace-wide
   items remain in the Founder inbox/header badge and are never copied into an
   arbitrary current or newly created chat. Text is a bounded domain projection,
   never raw email/page or candidate content.
...
- a general conversation cannot request a broader contributor set than the
  server selected;
- every run, wait, approval, receipt, and safe inbox item shown inline must be
  causally bound to the requested owned session. Missing origin is not treated
  as “latest” or “current”; it stays global until explicitly resolved;
...
- `since` defaults to the last locally acknowledged brief time for that exact
  conversation, is supplied as a hint only, and is clamped server-side to 30
  days;
- **Hide** is client-local and changes no durable work. **Dismiss** on a specific
  Founder inbox item calls the server and durably dismisses that item everywhere;
- if all contributors fail, the UI says current work could not be loaded. It
  never substitutes memory or transcript summarization. A partial contributor
  failure is rendered as a separate compact status, not as another notification.
...
- M1 inline delivery is additionally bound to the exact owned session. Global or
  ambiguous inbox items stay in the Founder inbox until resolved; they are never
  repeated in unrelated/new conversations. Local Hide and durable per-item
  Dismiss are separate controls.
